...
public class GlobalExceptionHandler {
	...
	//2. 第二關：Controller 邏輯接收如果第一關通過了，Controller 才會正式接到這個 m 物件，並呼叫 memberService.rigister(m)。
	
	//3. 第三關：Service 業務校驗 (Service/Logic Level)進入 MemberServiceImpl.java 後，才會執行你寫的邏輯檢查。
			//觸發點：你手動寫的 throw new MemberException(...)。行為：檢查驗證碼是否過期、帳號是否重複。結果：如果這裡失敗，會拋出 MemberException，
